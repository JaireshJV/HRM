...
import { ComplaintsReportTable  } from './ComplaintsReportTable'
...
import { getComplaint, selectAllComplaint } from '../../../../../Store/CustomSlice/EmployeeSlice'


export const ComplaintsReportForm = () => {

  const [dataSource, setDataSource] = useState([]);
  const [dateRange, setDateRange] = useState([]);
  const AllComplaints = useSelector(selectAllComplaint)
 ...
 useEffect(() => {
  ...
  const onFinish = (values) => {
    ...
    ReportComplaint(ConvertedValue)

  };

  const ReportComplaint = (values) => {
    request.post('complaints/date', values)
  ...
  return (
    <Fragment>
      ...
      <ComplaintsReportTable dataSource={dataSource}/>
    </Fragment>
  )
}
