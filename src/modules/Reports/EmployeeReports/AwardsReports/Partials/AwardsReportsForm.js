...
import { getAwards , selectAllAwards} from '../../../../../Store/CustomSlice/EmployeeSlice'
import { AwardsReportTable } from './AwardsReportsTable'


export const AwardsReportForm = () => {

  const [dataSource, setDataSource] = useState([]);
  const [dateRange, setDateRange] = useState([]);
  const AllAwards = useSelector(selectAllAwards)
 ...
 useEffect(() => {
  ...
  const onFinish = (values) => {
    ...
    ReportAwards(ConvertedValue)

  };

  const ReportAwards = (values) => {
    request.post('awards/date', values)
  ...
  return (
    <Fragment>
      ...
      <AwardsReportTable dataSource={dataSource}/>
    </Fragment>
  )
}
